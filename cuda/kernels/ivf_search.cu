...
__global__ void coarse_search_kernel(const float* __restrict__ d_queries,
                                     ...
                                     int nlist, int dim, int n_queries) {
    int centroid_idx = blockIdx.x * blockDim.x + threadIdx.x;
    int query_idx    = blockIdx.y;
    if (centroid_idx >= nlist || query_idx >= n_queries) return;
    ...
    #pragma unroll 4
    for (int i = 0; i < dim; i++) {
        float diff = s_query[i] - d_centroids[centroid_idx * dim + i];
        sum += diff * diff;
    }
    d_coarse_dists[query_idx * nlist + centroid_idx] = sum;
}

__global__ void ivf_final_topk_kernel(
    ...
    const int*   __restrict__ d_selected_clusters,
    int*   __restrict__ d_out_indices,
    float* __restrict__ d_out_distances,
    int n_queries, int k, int nprobe, int dim) {
    ...
    int   heap_idx [MAX_K];
    for (int i = 0; i < k; i++) { heap_dist[i] = FLT_MAX; heap_idx[i] = -1; }
    ...
    for (int p = 0; p < nprobe; p++) {
        ...
        for (int v = start + threadIdx.x; v < end; v += blockDim.x) {
            ...
            #pragma unroll 4
            for (int j = 0; j < dim; j++) {
                float diff = s_query[j] - d_inverted_vectors[(size_t)v * dim + j];
                dist += diff * diff;
            }
...
extern "C" void search_ivf(const float* d_queries,
    ...
    if (k > MAX_K) {
        fprintf(stderr, "search_ivf: k=%d exceeds MAX_K=%d. "
                        "Raise MAX_K and recompile.\n", k, MAX_K);
        return;
    }
    ...
    CUDA_CHECK(cudaMalloc(&d_coarse_dists,
                          (size_t)n_queries * index->nlist * sizeof(float)));
    {
        dim3 block(THREADS_PER_BLOCK);
        dim3 grid((index->nlist + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK,
                  n_queries);
    ...
    std::vector<int> h_selected((size_t)n_queries * nprobe);
    {
        thrust::device_vector<int> cluster_ids(index->nlist);
        for (int q = 0; q < n_queries; q++) {
            thrust::sequence(cluster_ids.begin(), cluster_ids.end());
            thrust::sort_by_key(
                thrust::device_pointer_cast(d_coarse_dists + q * index->nlist),
                thrust::device_pointer_cast(d_coarse_dists + q * index->nlist + index->nlist),
                cluster_ids.begin());

            thrust::copy(cluster_ids.begin(),
                         cluster_ids.begin() + nprobe,
                         h_selected.begin() + q * nprobe);
        }
    }

    int* d_selected_clusters = nullptr;
    CUDA_CHECK(cudaMalloc(&d_selected_clusters,
                          (size_t)n_queries * nprobe * sizeof(int)));
    CUDA_CHECK(cudaMemcpy(d_selected_clusters, h_selected.data(),
                          (size_t)n_queries * nprobe * sizeof(int),
                          cudaMemcpyHostToDevice));

    // 3. Fine search + final top-k
    {
